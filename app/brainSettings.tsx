...
const BrainSettingsPage = () => {
  ...
  useEffect(() => {
    const loadApi = async () => {
      try {
        ...
        if (apiValue) {
          console.log("Latest session URL from storage:", apiValue);
          setApi(apiValue);
        } else {
          console.log("No session URL found in storage");
          setIsLoading(false);
        }
      } catch (error) {
  ...
  const fetchSettings = async () => {
    ...
    try {
      // Fetch refresh rate
      console.log(
        `Fetching refresh rate from: ${api}/v1/burst_engine/stimulation_period`
      );
      const refreshResponse = await fetch(
        `${api}/v1/burst_engine/stimulation_period`
      );
      ...
      if (refreshResponse.ok) {
        ...
        console.error(
          `Error fetching refresh rate: ${refreshStatusCode}`,
          errorText
        );
      }
      ...
      if (plasticityResponse.ok) {
        ...
        console.error(
          `Error fetching plasticity queue: ${plasticityStatusCode}`,
          errorText
        );
      }
      ...
      console.log(
        `Fetching sampling rate from: ${api}/v1/system/cortical_area_visualization_skip_rate`
      );
      const samplingResponse = await fetch(
        `${api}/v1/system/cortical_area_visualization_skip_rate`
      );
      ...
      if (samplingResponse.ok) {
        ...
        console.error(
          `Error fetching sampling rate: ${samplingStatusCode}`,
          errorText
        );
      }
    } catch (error) {
  ...
  const validateFloat = (text) => {
    ...
    return text.replace(/[^0-9.]/g, "").replace(/(\..*)\./g, "$1"); // Ensuring only one decimal point
  };
  ...
  const applyChanges = async () => {
    ...
    try {
      ...
      console.log(
        `Sending POST to ${api}/v1/burst_engine/stimulation_period with payload:`,
      ...
      try {
        ...
        console.log(
          `Plasticity queue response data: ${plasticityResponseText}`
        );
      } catch (e) {
        plasticityResponseText = await plasticityResponse.text();
        console.log(
          `Plasticity queue response text: ${plasticityResponseText}`
        );
      }
      ...
      const samplingPayload = {
        cortical_viz_skip_rate: parseFloat(samplingRate),
      };
      console.log(
        `Sending PUT to ${api}/v1/system/cortical_area_visualization_skip_rate with payload:`,
        JSON.stringify(samplingPayload)
      );
      const samplingResponse = await fetch(
        `${api}/v1/system/cortical_area_visualization_skip_rate`,
        {
          ...
          <ScrollView contentContainerStyle={styles.scrollContent}>
            ...
            <View style={styles.settingRow}>
              <Text style={styles.settingLabel}>
                Visualization sampling rate (Hz)
              </Text>
              <TextInput
                style={styles.settingInput}
                value={samplingRate}
