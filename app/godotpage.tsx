...
import initializeSocket, { sendData } from "./websocket";
import { Dimensions } from "react-native";

export default function GodotPage() {
  const [godot, onGodotChange] = useState("");
  const [menuVisible, setMenuVisible] = useState(false); // state for hamburger menu dropdown
  ...
  const plugGodot = async () => {
    ...
    //8
    const godotLink =
      "https://storage.googleapis.com/nrs_brain_visualizer/1738016771/index.html?ip_address=" +
      concatLink +
      "&port_disabled=true&websocket_url=" +
      wssLink +
      "/p9055&http_type=HTTPS://";
