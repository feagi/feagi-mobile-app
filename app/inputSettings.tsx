...
const InputSettingsPage = () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data) {
          ...
          if (
            data.central_vision_resolution &&
            Array.isArray(data.central_vision_resolution) &&
            data.central_vision_resolution.length >= 2
          ) {
          ...
          if (
            data.peripheral_vision_resolution &&
            Array.isArray(data.peripheral_vision_resolution) &&
            data.peripheral_vision_resolution.length >= 2
          ) {
            setPeripheralResX(
              data.peripheral_vision_resolution[0]?.toString() || ""
            );
            setPeripheralResY(
              data.peripheral_vision_resolution[1]?.toString() || ""
            );
          }

          // single value fields, safely checking for null
          if (
            data.flicker_period !== null &&
            data.hasOwnProperty("flicker_period")
          ) {
          ...
          if (
            ...
            data.hasOwnProperty("eccentricity")
          ) {
            if (
              Array.isArray(data.eccentricity) &&
              data.eccentricity.length >= 2
            ) {
          ...
          if (
            data.pixel_change_limit !== null &&
            data.hasOwnProperty("pixel_change_limit")
          ) {
            setPixelChangeThreshold(
              convertDecimalToPercent(data.pixel_change_limit)
            );
          } else {
        ...
        console.error(
          `Error fetching vision settings: ${statusCode}`,
          errorText
        );
      }
    } catch (error) {
  ...
  const applyChanges = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        eccentricity:
          eccentricityX && eccentricityY
            ? [
                convertPercentToDecimal(eccentricityX),
                convertPercentToDecimal(eccentricityY),
              ]
            : null,
        modulation:
          modulationX && modulationY
            ? [
                convertPercentToDecimal(modulationX),
                convertPercentToDecimal(modulationY),
              ]
            : null,
      ...
      console.log(
        `Sending POST to ${api}/v1/input/vision with payload:`,
      ...
      if (response.ok) {
        ...
        console.error(
          `Failed to update vision settings: ${status}`,
          responseText
        );
          ...
          >
            ...
            <View style={styles.settingRow}>
              ...
              <TextInput
                ...
                onChangeText={(text) =>
                  setPixelChangeThreshold(validateInt(text))
                }
                keyboardType="numeric"
                placeholderTextColor="#aaa"
              />
